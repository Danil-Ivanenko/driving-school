...
interface StudentMarkDialogProps {
    userId: number;
    userName: string;
    taskId: string;
    onClose: () => void;
...
const StudentMarkDialog: React.FC<StudentMarkDialogProps> = ({
    userId,
    userName,
    taskId,
    onClose,
    ...
    const loadCurrentMark = async () => {
        const response = await api.getUserMark(userId, taskId);
        if (response) {
            setCurrentMark(response.mark);
    ...
    const handleSubmit = async () => {
        ...
        try {
            await api.setUserMark(userId, taskId, mark);
            onSuccess();
            onClose();
        } catch (err) {
...
export default StudentMarkDialog;